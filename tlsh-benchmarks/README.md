...
For a longer local measurement of the byte-array benchmark, override the JMH options:
...
The first benchmark measures complete `Tlsh.hash(byte[])` calls for 256-byte, 4 KiB, and 1 MiB
inputs. Input generation happens once before measurement, and every result is consumed so the JVM
cannot discard the hashing work as unused. Compare results only when the JDK, hardware, power mode,
JMH options, and other relevant environment details are recorded alongside them.
