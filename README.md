...
distance modes. CI additionally checks out the [official TLSH `5.0.0` test
corpus](https://github.com/trendmicro/tlsh/tree/5.0.0/Testing) and verifies full digests for real
example files plus official distance scores. The corpus remains in the CI workspace and is not
copied into this repository. The supported format is the 128-bucket, one-byte-checksum `T1`
variant.
